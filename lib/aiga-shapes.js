  ...
  var squareApp = new Aiga.FunWithShapes(raphael, {
    ...
    bootstrap: function() {
      ...
      for (var i=0; i < squareDimensions.length; i++) {
        shapes.push(scope.options.createShape(scope.bg, squareDimensions[i]));
      }
      return shapes;
    },
    ...
    bootstrap: function() {
      ...
      (function randomSpinning(triangles) {
        ...
        for (var i=0; i < count; i++) {
          if (Math.random() < 0.10) { Aiga.fx.rotator.call(triangles[i]); }
        }
        setTimeout(function() { randomSpinning(triangles); }, 5000);
      })(shapes);
      return shapes;
    }
  });
