...
$(document).ready(function() {
  ...
  var raphael = Raphael(document.getElementById("example"), "100%", "100%");

  var squareApp = new Aiga.FunWithShapes(raphael, {
    width: pageWidth, height: pageHeight, depth: 4, density: 0.25,
    createShape: function(raphael, grid) {
      var attrs = {"stroke":0, "fill":"#8DC63F"};
      var side = grid.w < grid.h ? grid.w : grid.h;
      return raphael.rect(grid.x, grid.y, side, side).attr(attrs).mouseover(Aiga.fx.pulsator)
    },
    boundingBox: function(shape) { return shape.getBBox(); },
    mouseover: function(shape, ev) { Aiga.fx.pulsator.call(shape, ev); }
  });

  var triangleApp = new Aiga.FunWithShapes(raphael, {
    width: pageWidth, height: pageHeight, depth: 4, density: 0.5,
    createShape: function(raphael, grid) {
      var attrs = {"stroke":0, "fill":"#44C8F5"};
      return raphael.triangle((grid.x + grid.w/2), grid.y, grid.h).
        attr(attrs).mouseover(Aiga.fx.rotator);
    },
    boundingBox: function(shape) { return shape.getBBox(); },
    mouseover: function(shape, ev) { Aiga.fx.rotator.call(shape, ev); }
  });

  var circleApp = new Aiga.FunWithShapes(raphael, {
    width: pageWidth, height: pageHeight, depth: 3, density: 0.5,
    createShape: function(raphael, grid) {
      var moonCount = Math.ceil(Math.random() * 10);
      var cx = (grid.x + grid.w/2), cy = (grid.y + grid.h/2), radius = grid.w/4;
      var system = new Aiga.PlanetarySystem(raphael, {
        moonCount: moonCount, cx: cx, cy: cy, radius: radius, color: "yellow", stroke: 0
      });
      system.draw();
      return system;
    },
    boundingBox: function(shape) { return shape.planet.getBBox(); },
    mouseover: function(shape, ev) { shape.orbit(); }
  });

  // Making it all go
  var currentApp = triangleApp;
  $("a[href=#square]").click(function() { (currentApp = squareApp).draw(); return false; });
  $("a[href=#triangle]").click(function() { (currentApp = triangleApp).draw(); return false; });
  $("a[href=#circle]").click(function() { (currentApp = circleApp).draw(); return false; });
  currentApp.draw();

  // Deal with mouseovers for movements not over the canvas; pass them along to the stuff below
  $("*:not(#example)").mousemove(function(ev) { currentApp.checkMouseover(ev); });
});
